    ...
    return (
        isLoading ? <div className={styles.loadingDiv}>
            ...
            <div className={styles.projectList}>
                    ...
                    {listProject.map((project, index) => {
                        return <div key={index} className={styles.projectBox} >
                            <Link className={styles.projectBox_link} to={`/projectDetail/${project?.id}`}>
                                <div className={styles.project} style={project?.background == "black" ? { background: `${project?.background}`, color: 'white' } : { background: `${project?.background}` }}>
                ...
                {
                    createFormShow ? <CreateProjectContainer listProject={listProject} setCreateFormShow={setCreateFormShow} /> : <></>
                }
            </div>
    )
}
